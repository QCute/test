...
dispatch(State, <<Length:16, Protocol:16, Binary:Length/binary, Rest/binary>>) ->
...
%% key map
header_key_map(<<"cache-control">>) ->
    'Cache-Control';
header_key_map(<<"connection">>) ->
    'Connection';
header_key_map(<<"date">>) ->
    'Date';
header_key_map(<<"pragma">>) ->
    'Pragma';
header_key_map(<<"transfer-encoding">>) ->
    'Transfer-Encoding';
header_key_map(<<"upgrade">>) ->
    'Upgrade';
header_key_map(<<"via">>) ->
    'Via';
header_key_map(<<"accept">>) ->
    'Accept';
header_key_map(<<"accept-charset">>) ->
    'Accept-Charset';
header_key_map(<<"accept-encoding">>) ->
    'Accept-Encoding';
header_key_map(<<"accept-language">>) ->
    'Accept-Language';
header_key_map(<<"authorization">>) ->
    'Authorization';
header_key_map(<<"from">>) ->
    'From';
header_key_map(<<"host">>) ->
    'Host';
header_key_map(<<"if-modified-since">>) ->
    'If-Modified-Since';
header_key_map(<<"if-match">>) ->
    'If-Match';
header_key_map(<<"if-none-match">>) ->
    'If-None-Match';
header_key_map(<<"if-range">>) ->
    'If-Range';
header_key_map(<<"if-unmodified-since">>) ->
    'If-Unmodified-Since';
header_key_map(<<"max-forwards">>) ->
    'Max-Forwards';
header_key_map(<<"proxy-authorization">>) ->
    'Proxy-Authorization';
header_key_map(<<"range">>) ->
    'Range';
header_key_map(<<"referer">>) ->
    'Referer';
header_key_map(<<"user-agent">>) ->
    'User-Agent';
header_key_map(<<"age">>) ->
    'Age';
header_key_map(<<"location">>) ->
    'Location';
header_key_map(<<"proxy-authenticate">>) ->
    'Proxy-Authenticate';
header_key_map(<<"public">>) ->
    'Public';
header_key_map(<<"retry-after">>) ->
    'Retry-After';
header_key_map(<<"server">>) ->
    'Server';
header_key_map(<<"vary">>) ->
    'Vary';
header_key_map(<<"warning">>) ->
    'Warning';
header_key_map(<<"www-authenticate">>) ->
    'Www-Authenticate';
header_key_map(<<"allow">>) ->
    'Allow';
header_key_map(<<"content-base">>) ->
    'Content-Base';
header_key_map(<<"content-encoding">>) ->
    'Content-Encoding';
header_key_map(<<"content-language">>) ->
    'Content-Language';
header_key_map(<<"content-length">>) ->
    'Content-Length';
header_key_map(<<"content-location">>) ->
    'Content-Location';
header_key_map(<<"content-md5">>) ->
    'Content-Md5';
header_key_map(<<"content-range">>) ->
    'Content-Range';
header_key_map(<<"content-type">>) ->
    'Content-Type';
header_key_map(<<"etag">>) ->
    'Etag';
header_key_map(<<"expires">>) ->
    'Expires';
header_key_map(<<"last-modified">>) ->
    'Last-Modified';
header_key_map(<<"accept-ranges">>) ->
    'Accept-Ranges';
header_key_map(<<"set-cookie">>) ->
    'Set-Cookie';
header_key_map(<<"set-cookie2">>) ->
    'Set-Cookie2';
header_key_map(<<"x-forwarded-for">>) ->
    'X-Forwarded-For';
header_key_map(<<"cookie">>) ->
    'Cookie';
header_key_map(<<"keep-alive">>) ->
    'Keep-Alive';
header_key_map(<<"proxy-connection">>) ->
    'Proxy-Connection';
%% web socket addon
header_key_map(<<"sec-websocket-key">>) ->
    'Sec-WebSocket-Key';
header_key_map(<<"sec-websocket-version">>) ->
    'Sec-WebSocket-Version';
header_key_map(<<"sec-websocket-extensions">>) ->
    'Sec-WebSocket-Extensions';
header_key_map(Key) ->
    Key.
